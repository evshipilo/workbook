...
// Инкапсуляция ускоряет процесс разработки: слабо связанные друг с другом компоненты
...
// 3) Полиморфизм - этот принцип неразрывно связан с другим принципом ООП – наследованием. Классы-наследники наследуют функциональность
// классов-родителей но конкретная реализация этой функциональности может быть разной. (переопределение методов и конструктора в наследниках)
...
//  1)Принцип единственной ответственности (Single Responsibility Principle, SRP) - означает, при изменении какого-либо бизнес требования к
//  приложению вся функциональность которая требует изменения должна быть собрана в 1 модуле. У модуля должна быть только одна причина для
//  изменения. Принцип позволяет уменьшить количество кода, который  нужно  менять при изменении бизнес-правил.
...
// TODO Наблюдатель — это поведенческий паттерн проектирования, который создаёт механизм подписки, позволяющий одним объектам следить и
