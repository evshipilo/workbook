...
// OOP is a programming methodology based on the representation of a program as a collection of objects,
// each of which is an instance of a certain class, and classes form an inheritance hierarchy. Principles:
// 1) Encapsulation - hiding the internal implementation of an object from other objects and providing a set of methods for interacting with it (API).
// Encapsulation speeds up the development process: loosely coupled components can be developed, tested and supplemented
// independently and can be reused.
// 2) Inheritance - description of new classes based on an existing (parent) one. At the same time, the properties and functionality of the parent class
// borrowed by a new class.
// 3) Polymorphism - successor classes inherit the functionality of parent classes, but the specific implementation of this functionality may be
// different. (redefinition of methods and constructor in inheritors)
// 4) Abstraction — for each object we set the minimum number of properties that will allow us to solve the problem.
